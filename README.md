...
The Docker image contains both environments from [Step 1](#step-1--set-up-environments) and the ExecuTorch runtime. To include the evaluated source revision and its latest `check` and `smoke` commands, build the [Dockerfile](Dockerfile) from that checkout. The published `v1.0` image has not been revalidated for these newer commands. Skip Step 1 when using a prepared image.
